...
constexpr rekt::flat_set<int, 5> ints_s(rekt::array<int, 5>({ 2, 3, 1, 4, 0 }));
...
constexpr int l = rekt::lexicographic_compare(ints_s2.to_array(), rekt::array<int, 6>({ 0, 1, 2, 2, 3, 4 }));


int main()
{
  auto a = rekt::array<int, 4>({ 0, 1, 2, 3 });
  auto b = ints_s2.to_array();
  int sum = l;
  for (auto i : b)
  {
    sum += i;
